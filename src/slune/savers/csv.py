...
class SaverCsv(BaseSaver):
    ...
    def read(self, params: dict, metric_name: str, select_by: str ='max', avg: bool =True) -> Tuple[dict, float]:
        ...
        paths = get_all_paths(dict_to_strings(params), root_directory=self.root_dir)
    ...
    def getset_current_path(self, params:dict=None, save:bool=True) -> str:
        ...
        if params is not None:
            ...
            if self.current_params is None:
                raise ValueError('SaverCsv.current_params is None, please provide parameters to get the current path.')
            elif self.current_path is None:
                raise Exception('SaverCsv.current_path is None, please provide parameters to create a path.')
